...
export interface getArchivedLotsParams
	extends Partial<ISearchParams & { page: number }> {}

export type getArchivedLotsConfig = FetchesRequestConfig<getArchivedLotsParams>

export const getArchivedLots = ({
	params,
	config,
}: getArchivedLotsConfig): Promise<
	FetchesResponse<{ pagination: IPagination; lots: TLot[] }>
> => {
	return $Api.get('/auction-vehicles/archive-lots/', {
		params: {
			make:
