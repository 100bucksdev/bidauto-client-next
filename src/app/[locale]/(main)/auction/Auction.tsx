...
const AuctionPage = () => {
	...
	const {
		...
		hasNextPage,
		fetchNextPage,
	} = useGetAllAuctionVehicles({ search, category: id || '0' })
	...
	const { data, isLoading } = useGetAuctionFirstFoutVehiclesEveryCategory()
								...
								<div>
									...
									<div className='flex flex-wrap justify-center mt-12 px-48 max-md:px-6 max-xl:px-12 gap-8'>
										{el.vehicles.map(vehicle => (
											<AuctionCard car={vehicle} key={vehicle.id} />
										))}
									</div>
									{el.vehicles.length === 4 && (
										<div className='flex justify-center mt-10'>
											<button
												onClick={() =>
													path(`/auction/${el.category.slug}/${el.category.id}`)
												}
